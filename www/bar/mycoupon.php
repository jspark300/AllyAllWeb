...
/*$sql = "select datediff(date_add(str_to_date(end_date,'%Y%m%d'), INTERVAL 31 DAY),now()) gr  from t_give where id=$id and status=2";
$res = sql_fetch($sql);
$last_count = $res[gr];
if($last_count < 0)
{
...
?>
		...
		<div>
			<dl>
				...
				<dd>
					...
					<? } ?>
					<p>상태 : <?
									if($give[status]==2 || $give[status]==110) 
										echo "미사용"; 
	...
	<section class="myentry">
		...
		<ul class="btn3">
			<li><a href="javascript:shop_ceo_confirm();">주인장확인(쿠폰사용완료)</a></li>
			<li><a href="mycoupon_list.php">쿠폰목록</a></li>
