...
if ($w == "u") {
    for ($i=0; $i<$file['count']; $i++) {
        if ($file[$i]['source']) {
			...
			if ($is_file_content) {
				$file_script .= "<div class='col-sm-5'><div class='form-group'><input type='text' name='bf_content[$i]' value='".addslashes(get_text($row['bf_content']))."' class='form-control input-sm' placeholder='이미지에 대한 내용을 입력해 주세요.'></div></div>";
			}
			$file_script .= "<div class='col-sm-12'><div class='form-group'><label class='checkbox-inline'>{$file[$i]['source']}({$file[$i]['size']}) </label></div></div>";
...
$action_url = "/board_write_update.php";
...
<head>
	...
	<link href="/skin/board/basic/style.css" rel="stylesheet">
	<link href="/thema/assets/bs3/css/bootstrap.min.css" rel="stylesheet">

	<?
	add_stylesheet('<link rel="stylesheet" href="'.THEMA_URL.'/assets/bs3/css/bootstrap.min.css" type="text/css" media="screen">',0);
	add_stylesheet('<link rel="stylesheet" href="/skin/board/basic/style.css" media="screen">', 0);
	?>
<script type="text/javascript">
	function gosave()
	{
		if(trim(document.reg.wr_subject.value) == "")
		{
			alert('제목을 입력해 주세요.');
			return;
		}
...
//		if(trim(document.reg.wr_content.value) == "")
//		{
//			alert('내용을 입력해 주세요.');
//			return;
//		}
...
<body>
	...
	<section class="search">
		<div class="tab_wrap">
			<ul class="u3">
				<li><a href="javascript:ch('notice');" <?if($bo_table=="notice") echo "class='on'"; ?> >공지</a></li>
				<li><a href="javascript:ch('qa');" <?if($bo_table=="qa") echo "class='on'"; ?> >문의</a></li>
				<li><a href="javascript:ch('guide');" <?if($bo_table=="guide") echo "class='on'"; ?> >안내</a></li>
				<li><a href="javascript:ch('public');" <?if($bo_table=="public") echo "class='on'"; ?> >홍보</a></li>
			</ul>
		</div>
	</section>
<!--form name="reg" method="post" charset="utf-8" action="board_write_ok.php">
...
<?php
	...
	<section class="shop_menulist">
		<ul class="input_wrap">
			<li>
				<p class="tit_txt">● 제목</p>
				<div class="input">
					<input type="text" name="wr_subject" placeholder="제목 입력" value="<?php echo $subject ?>">
				</div>
			</li>
			<!--li>
			...
			<li>
				<p class="tit_txt">● 내용</p>
				<div class="input">
					<!-- textarea placeholder="내용 입력" name="content"><?=$rs[wr_content]?></textarea-->
...
<?php if ($is_file) { ?>
		...
		#variableFiles .checkbox-inline { padding-top:0px; font-weight:normal; }
			...
			<li>
				<p class="tit_txt">● 첨부파일 	<button class="btn2x" type="button" onclick="add_file();"><i class="fa fa-plus-circle fa-lg"></i> 추가하기</button>
				<button class="btn2x" type="button" onclick="del_file();"><i class="fa fa-times-circle fa-lg"></i> 삭제하기</button></p>
				<div class="input text" >
					<!--input type='file' class='form-control input-sm' name='bf_file[]' title='파일 용량 <?php echo $upload_max_filesize; ?> 이하만 업로드 가능'>
					<input type='file' class='form-control input-sm' name='bf_file[]' title='파일 용량 <?php echo $upload_max_filesize; ?> 이하만 업로드 가능' -->
		...
		function add_file(delete_code) {
			...
			objContent += "<div class='col-sm-7'><div class='form-group'><div class='input-group input-group-sm'><span class='input-group-addon' style='display:none;'>파일 "+objNum+"</span><input type='file' class='form-control input-sm' name='bf_file[]' title='파일 용량 <?php echo $upload_max_filesize; ?> 이하만 업로드 가능'></div></div></div>";
			if (delete_code) {
				objContent += delete_code;
		    } else {
				<?php if ($is_file_content) { ?>
				objContent += "<div class='col-sm-5'><div class='form-group'><input type='text'name='bf_content[]' class='form-control input-sm' placeholder='이미지에 대한 내용을 입력해 주세요.'></div></div>";
				<?php } ?>
				;
			}
			objContent += "</div>";
	...
	<?php } ?>
		...
		<div class="btn1">
			...
			<a href="/board_list.php?bo_table=<?php echo $bo_table ?>" class="btn btn-black btn-sm" role="button">취소</a>
		</div>
	</section>
</form>
<script type="text/javascript">
