...
{
	echo "<script>alert('업체권한이 필요합니다.');history.back();</script>";
	exit;
}
...
$sql = "select * ,(select name from t_comp where id=com_id) comstr  from t_com_order where  $wh ($where) order by id desc limit $listc";
$res = sql_query($sql);
...
<script> 
  ...
  function addlist(){ 
        ...
        listcount+=10; 
        $.post("/get_comorder.php",{"count" : listcount, "t":<?=$t?$t:0;?>},function(data){ 
