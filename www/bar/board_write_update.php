...
if ($wr_subject == '') {
    $msg[] = '<strong>제목</strong>을 입력해 주세요.';
}
...
if ($wr_content == '') {
    $msg[] = '<strong>내용</strong>을 입력해 주세요.';
}
...
if ($w == '' || $w == 'r') {
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($write['mb_id'])
            alert('로그인 후 수정해 주세요.', './login.php?url='.urlencode($return_url));
    }
...
for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
    ...
    if (is_uploaded_file($tmp_file)) {
        ...
        if (!$is_admin && $filesize > $board['bo_upload_size']) {
            $file_upload_msg .= '첨부는 '.number_format($board['bo_upload_size']/1000000).'MB 이하의 파일만 업로드 해주세요.\\n';
            continue;
        }
...
if ($file_upload_msg)
    alert($file_upload_msg, '/board_list.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr);
else
    goto_url('/board_list.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);
?>
