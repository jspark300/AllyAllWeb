...
<head>
	...
	<meta property="og:title" content="ALLYALL : <?=$res[name]?>" />
	<meta property="og:url" content="http://allyall.co.kr/shop_detail.php?id=<?=$id?>&t=<?=time()?>" />
	<meta property="og:description" content="<?=$res['description']?>" />
	<meta property="og:image" content="http://allyall.co.kr/data/shop/<?=$res[img_id]?>/1.jpg" />		
	<title>ALLYAll</title>
...
<header class="header2">
		<div class="btn_back"><a href="shop_list.php?c1=<?=$c1?>&c2=<?=$c2?>&lc=<?=$lc?>&sc=<?=$sc?>&t=<?=time();?>"></a>
		</div>
		<div class="txt_tit">
			<p><?=$res['name']?></p>
		</div>
...
<?if($c1<21) {?>
	<section class="shop_detail">
		<?
			...
			if($give) {
				$total_enter = floor($give[price]/10000)*50;
		...
		<div class="btn_wrap">
			<ul>
				...
				<li><a href="shop_eval.php?id=<?=$id?>">평가</a></li>
...
?>
					<p>거리 <?=number_format($res[distance])?>m, 단골 <?=$res['regular_count']?>, 조회 <?=$res['view']?>, 콜수 <?=$call_count?>
		...
		<ul>
			<li class="eval">
				<dl>
<?
$icon_p = floor($res[eva_point]);
?>							<dt><span class="e<?=$icon_p?>"><i></i></span><strong><?=$res[eva_point]?>점</strong></dt>
					<dd>
<?
$sql = "select round(avg(eva1),1) c1,round(avg(eva2),1) c2,round(avg(eva3),1) c3,round(avg(eva4),1) c4, count(id) c from t_com_eva where com_id=$id";
$rx = sql_fetch($sql);

?>
						<p><span class="e5"><!--i></i--></span> <em class="on">맛집</em> <?=$rx[c1]?></p>
						<p><span class="e3"><!--i></i--></span> <em class="on">신속</em> <?=$rx[c2]?></p>
						<p><span class="e5"><!--i></i--></span> <em class="on">친절</em> <?=$rx[c3]?></p>
						<p><span class="e5"><!--i></i--></span> <em class="on">청결</em> <?=$rx[c4]?></p>
						<div><strong><?=$rx[c]?>명 참여</strong> (5.0만점)</div>
					</dd>
				</dl>
			</li>
    ...
    Kakao.Link.createTalkLinkButton({
	 ...
	 function shareStory() {
        Kakao.Story.share({
			...
			text: '<?=$res[name]?> : 올이올 세상의 모든 동맹 ;)'
        });
      }
	  function facebook_share()
	  {
		  var u = encodeURIComponent("http://allyall.co.kr/shop_detail.php?id=<?=$id?>&t=<?=time()?>");
