...
<? 
$sql = "select count(mb_no) c from g5_member where  mb_level=4";
$cx = sql_fetch($sql);
$comment_count = $cx[c];
...
?>    
		...
		<div class="reply_tit">
            영업회원 : <?=$comment_count?> 명
        </div>
        <ul class="reply_list" id="listbody">
		
	<? 

$sql = "select * from g5_member where  mb_level=4 order by mb_no desc  limit 10";
$rs = sql_query($sql);
...
<script>
  var listcount=0; 
   function addlist(){ 
        listcount+=10; 
        $.post("/get_bz_member_list.php",{"count" : listcount},function(data){ 
