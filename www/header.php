	<?php include_once("analyticstracking.php") ?>
	<header class="header1">
		<div class="d1">
			<h1><a href="shop_list.php?c1=<?//=$c1?>"><img src="images/logo_sub.png" alt="ALLYALL"></a></h1>
			<p style='float:left;width:60px;text-align:center;margin:12px 0 0 0;font-size:14px;'><?=$loc?></p>
			<nav>
				<ul class="btn_wrap">
					
					<li><a href="search.php" class="search">검색</a></li>
					<li><a href="locate.php" class="locate">위치</a></li>
					<li><a href="my_index.php" class="menu">메뉴</a></li>
				</ul>
			</nav>
		</div>
		<!--div class="search_wrap1 dp_none">
			<p><input type="search" placeholder="검색어 입력"></p> <a href="#1">검색</a>
		</div-->
	</header>
