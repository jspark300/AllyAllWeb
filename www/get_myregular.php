...
while ($row=sql_fetch_array($res)) {
	++$i;
	$sql = "select id,name,cate,cate_sub,img0,img_id,lat,lon,eva_point,eva_count,order_tel_st,order_sms_st,regular_count ".$dist." from t_comp where id=$row[com_id]";
	$rx = sql_fetch($sql);
	$cate = $rx['cate'];
	if($rx['img_id'] == "")
	{
...
<? $icon_p = floor($rx[eva_point]); ?>						
							<p class="eval">
								<span class="s1 e<?=$icon_p?>"><i></i><?=$rx[eva_point]?>점</span>
